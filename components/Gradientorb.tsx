...
export const GradientOrb = ({ isSpeaking = false, isLoading = false }) => {
  ...
  return (
    <div className="flex justify-center py-8 mb-6">
      <div
        className={clsx(
          "relative w-64 h-64 rounded-full transition-all duration-500 ease-in-out",
          {
            // Idle state - soft pulse
            "border-purple-200/50 animate-pulse-soft": isIdle,
            // Loading state - continuous pulse
            "border-purple-300/70 animate-pulse-loading": isLoading && !isSpeaking,
      ...
      <style jsx>{`
        ...
        .animate-pulse-loading {
          animation: pulse-loading 0.9s ease-in-out infinite;
        }
